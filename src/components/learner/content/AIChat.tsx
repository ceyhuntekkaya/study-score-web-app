...
export default function AIChat({ activeText, lessonPartName }: AIChatProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-lg border border-gray-200 shadow-sm">
          ...
          <div className="flex flex-col items-center justify-center h-full text-center">
            <div className="mb-4">
              <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center mb-3">
                <i className="feather-send w-6 h-6 text-blue-600"></i>
              </div>
            </div>
            ...
            {messages.map((message) => (
              <div
                ...
                className={`ai-chat-message ${message.role} ${
                  message.role === 'user'
                    ? 'bg-blue-500 text-white ml-auto max-w-[80%] rounded-lg p-3'
                    : 'bg-white text-gray-800 mr-auto max-w-[80%] rounded-lg p-3 shadow-sm'
                }`}
              >
                  ...
                  <>
                    ...
                    {message.isComplete === false && (
                      <div className="flex mt-3">
                        <LoadingDots size="md" color="primary" enhanced={true} />
                      </div>
                    )}
                  </>
                ) : (
      ...
      {/* Input alanı - sabit */}
      <div className="ai-chat-form-container border-t bg-white p-4">
        <form onSubmit={handleSubmit} className="ai-chat-form flex gap-2" ref={formRef}>
          <input
            ref={inputRef}
            type="text"
            value={input}
            ...
            className="ai-chat-input flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            disabled={isLoading}
          />
          <button
            type="submit"
            disabled={isLoading || input.trim() === '' || serviceStatus !== 'online'}
            className={`send-btn px-4 py-2 bg-blue-500 text-white rounded-lg flex items-center justify-center ${
          ...
          >
            {isLoading ? (
              ...
              <i className="feather-send w-5 h-5"></i>
            )}
          </button>
        </form>
      </div>
