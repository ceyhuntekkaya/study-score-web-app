...
export default function MultipleResponseTemplateForm({
  ...
  const updateChoice = (index: number, field: string, value: any) => {
    ...
    // Multiple response: allow multiple isCorrect. Only enforce at least one correct.
    if (field === "isCorrect" && value === false) {
      const wouldHaveNone = updatedChoices.filter((c) => c.isCorrect).length === 0;
      if (wouldHaveNone && updatedChoices.length > 0) {
