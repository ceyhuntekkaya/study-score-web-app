...
export default function StudentsPage() {
  ...
  const columns: Column<User>[] = [
    {
      key: 'username',
      header: t('admin.learner.username') ?? 'Kullanıcı adı',
      sortable: true,
      render: (v) => (v ?? '—') as ReactNode,
    },
    {
      key: 'name',
      header: t('admin.learner.name') ?? 'Ad',
      sortable: true,
      render: (_, row) => [row.name, row.lastName].filter(Boolean).join(' ') || '—',
    },
    {
      key: 'email',
      header: t('common.email') ?? 'E-posta',
      sortable: true,
      render: (v) => (v ?? '—') as ReactNode,
    },
    {
      key: 'branch',
      header: t('admin.entity.branch') ?? 'Şube',
      sortable: false,
      ...
      actions: [
        {
          ...
          onClick: (item) => item.id && router.push(`/admin/dashboard/students/${item.id}`),
        },
        {
          label: (
            <>
          ...
          onClick: (item) => item.id && router.push(`/admin/dashboard/students/${item.id}/edit`),
        },
      ],
    },
  ];
